...
The DAG produces **both scales** in one run: `build_telecom_churn_100k`
feeds the 100k Expresso sample → `telecom_churn_100k.csv`;
`build_telecom_churn_full` feeds the raw 2 M Expresso →
`telecom_churn.csv`. Downstream tasks pick the scale via the
`USE_FULL_DATASET` Airflow Variable (`collect_telecom_paths` packs both
paths into a dict the downstream tasks unpick).
...
```
dags/churn_pipeline.py        # Airflow DAG (TaskFlow @task × 8)
...
The same table is available as a machine-readable CSV in
`outputs/metrics/scales_comparison.csv` (regenerated by
`scripts/05_train_model.py --all-scales`).

### Reproduce both scales in one command

`scripts/05_train_model.py` now supports a `--all-scales` flag that
trains on `telecom_churn_100k.csv` AND `telecom_churn.csv` (full 2 M)
back-to-back, saving suffixed artefacts so the two never overwrite
each other:

```bash
python scripts/05_train_model.py --all-scales --no-shap
# Produces:
#   models/churn_model_100k.joblib + churn_model_full.joblib
#   outputs/metrics/evaluation_metrics_{100k,full}.csv
#   outputs/metrics/feature_importances_{100k,full}.csv
#   outputs/metrics/scales_comparison.csv

python scripts/06_predict.py --all-scales
# Produces:
#   outputs/predictions/churn_predictions_{100k,full}.csv
```

The canonical `models/churn_model.joblib` is left at whichever scale ran
last, so the Streamlit dashboard + Flask API continue to work without
extra steps.
